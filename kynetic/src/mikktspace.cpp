...
static float LengthSquared(const SVec3 v) { return v.x * v.x + v.y * v.y + v.z * v.z; }
...
static float vdot(const SVec3 v1, const SVec3 v2) { return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z; }
...
tbool genTangSpaceDefault(const SMikkTSpaceContext* pContext) { return genTangSpace(pContext, 180.0f); }
                           ...
                           const int iR_in)
{
    ...
    else
    {
        ...
        if (iL_in < iR) MergeVertsFast(piTriList_in_and_out, pTmpVert, pContext, iL_in, iR);  // weld all left of fSep
        if (iL < iR_in)
                        ...
                        const int iNrTrianglesIn)
{
    ...
    for (f = 0; f < iNrTrianglesIn; f++)
    {
        ...
        if (NotZero(fSignedAreaSTx2))
        {
            ...
            if (NotZero(pTriInfos[f].fMagS) && NotZero(pTriInfos[f].fMagT)) pTriInfos[f].iFlag &= (~GROUP_WITH_ANY);
        }
    }
        ...
        for (i = 0; i < pGroup->iNrFaces; i++)  // triangles
        {
            ...
            for (j = 0; j < pGroup->iNrFaces; j++)
            {
...

                {
                    ...
                    if (bAny || bSameOrgFace || (fCosS > fThresCos && fCosT > fThresCos)) pTmpMembers[iMembers++] = t;
                }
            }
